			  ...
			  datareceived = xmlhttp.responseText;
			   ...
			   //loop through each object
					...
					$.each(obj, function(key,value) {
						//console.log("aaaaaa");
						console.log("current shift: "+currentshiftid+" shift value "+value.shiftid);
					   //currentshiftid=value.shiftid;
					
						   
						   if(newshift){
						   alert (value.shiftid+" AND "+currentshiftid);
						  //alert(value.fname+" "+value.lname+" "+value.status);
						  $('#namelist').append('<tr style="text-align:center;background-color: #3d3d29;"  > <td id="'+value.id+'" onClick="getselname(this)">'+'SHIFT: '+value.shiftid
						  +'<p> People required: '+value.required+'<p>'+
						  value.fname+" "+value.lname+" "+value.status+
						  '<p>'+'</td> </tr>');
						  var currsh=value.shiftid;
							$.each(shiftli, function(skey,svalue) {
						
							 $('#namelist').append('<tr><td><tp><b>'+svalue.fname+' '+svalue.lname+'shift '+svalue.shiftid+'<b></tp></td></tr>');
							});
						    ...
						    if(currentshiftid!=value.shiftid){
                             currentshiftid=value.shiftid;
							 newshift=false;
							}else {newshift=true;}
							//newshift=false;
